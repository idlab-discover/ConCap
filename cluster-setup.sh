...
docker exec kind-worker bash -c "apt update && apt install -y nfs-common" &
docker exec kind-worker2 bash -c "apt update && apt install -y nfs-common" &
docker exec kind-worker3 bash -c "apt update && apt install -y nfs-common" &
