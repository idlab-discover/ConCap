...
func (s *MultiTargetScenario) AttackPod() *apiv1.Pod {
	return BuildAttackerPod(s.Attacker.Name, s.Attacker, s.Name, s.Network)
}
...
func (s *MultiTargetScenario) TargetPod(index int) *apiv1.Pod {
	...
	return BuildTargetPodFromConfig(s.Targets[index], s.Name, index)
}
	...
	for _, target := range s.Targets {
		...
		for _, pod := range processingPods {
			...
			go func(pod *ProcessingPod, scenarioName string, targetName string, targetDir string, labels map[string]string) {
				...
				// Merge scenario labels with target-specific labels
				...
				mergedLabels["target"] = targetName

				err := pod.ProcessPcap(filepath.Join(targetDir, "dump.pcap"), scenarioName, targetDir, mergedLabels)
				if err != nil {
...
func (s *MultiTargetScenario) GetShellEnvVars() map[string]string {
	...
	for i, targetPodSpec := range s.Deployment.TargetPodSpecs {
		envVars[fmt.Sprintf("TARGET%d_IP", i+1)] = targetPodSpec.PodIP
	}
